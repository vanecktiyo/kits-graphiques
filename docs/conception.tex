...
\textbf{Version :} 0.7 (conception)\\[0.2cm]
\textbf{Date :} \today
}
\end{flushleft}
...
\textbf{texte} (accroche, sous-accroche, offre, paliers, date) et déduit quelques paramètres
...
\texttt{transversale}) sert à choisir l'image ; \texttt{sous\_titre} est une
sous-accroche optionnelle (1 à 3 mots) posée sous le titre, plus petite et plus légère
(ex. « Fenêtres » puis « sur-mesure ») ; \texttt{offre\_type}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{L'IA, concepteur-rédacteur (la créativité) :} à partir du brief,
        Claude rédige le copy (accroche, sous-accroche, offre, paliers, date) et \textbf{déduit} la
...
Elle rédige le copy (accroche, sous-accroche, offre, paliers, date) et \textbf{déduit} quelques
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Le texte est produit via l'\textbf{API Anthropic directe}. Le passage à
        \textbf{Vertex AI} (Claude dans Model Garden) a été \textbf{évalué} : il
        offrirait une intégration 100\,\% GCP-native (authentification par compte de
        service, facturation unifiée) et la bascule de code est triviale (client
        \texttt{AnthropicVertex}, même identifiant de modèle). Il est toutefois
        \textbf{écarté pour ce projet} : Claude y est un \textbf{produit partenaire},
        non activable avec un compte de facturation en \textbf{essai gratuit} (compte
        payant requis, hors crédits d'essai). L'API directe, \textbf{fonctionnellement
        identique}, a donc été conservée.
\end{itemize}
...
Le déploiement est \textbf{automatisé}. Le dépôt GitHub est connecté à \textbf{Cloud
Build} par un déclencheur : à chaque \texttt{git push} sur la branche \texttt{main}, une
recette versionnée (\texttt{cloudbuild.yaml}) reconstruit l'image (image simple, sans
attestation), la pousse dans Artifact Registry, puis redéploie automatiquement sur Cloud
Run. Plus aucune commande manuelle : la chaîne \emph{code $\rightarrow$ build
$\rightarrow$ registre $\rightarrow$ production} est entièrement déclenchée par le
\texttt{push}.
...
\hrule
\vspace{0.5em}
{\small\itshape Fin du cahier de conception (version 0.7). Version 0.7 : ajout du champ
\texttt{sous\_titre} (sous-accroche optionnelle sous le titre) et de l'\textbf{intégration
et déploiement continus} (CI/CD via Cloud Build : redéploiement automatique à chaque
\texttt{git push} sur \texttt{main}). Version 0.6 : ajout d'une
