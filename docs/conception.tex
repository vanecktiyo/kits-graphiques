...
\textbf{Version :} 0.4 (conception)\\[0.2cm]
\textbf{Date :} \today
}
\end{flushleft}
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{F1. Saisie du brief :} l'utilisateur décrit la mécanique
        commerciale (1.1), le message commercial (1.2), en option les illustrations
        (1.3 : style souhaité et demande éventuelle d'une image), et choisit la
        \textbf{couleur de fond} dans la palette de la charte (menu déroulant).
  ...
  \item \textbf{F3. Génération du texte :} le système transforme le brief en
        ...
        appel à l'action) et déduit la catégorie, le besoin d'image et, pour une
        opération transversale, le thème (ex. Black Friday).
...
l'offre ; \texttt{illustrer} indique si une image accompagne les pubs ;
\texttt{offre\_pastille} affiche la remise en badge de coin ;
\texttt{theme\_transverse} sélectionne le visuel d'une opération transversale (ex.
Black Friday). La \textbf{couleur de fond} ne vient pas de Claude : elle est choisie
dans le formulaire. Le \textbf{même message} est conservé sur tous les formats : les
gabarits adaptent la taille du texte aux petits formats.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{L'IA, concepteur-rédacteur (la créativité) :} à partir du brief
        (dont le champ \textbf{1.3 illustrations}), Claude rédige le copy et
        \textbf{déduit} la catégorie, le type d'offre, le besoin d'image et, pour une
        opération transversale, le thème. La \textbf{couleur de fond} est choisie par
        l'utilisateur dans le formulaire (plus prévisible). Claude ne génère pas de
        HTML : c'est le code qui rend.
\end{itemize}

Ce choix (l'IA \emph{rédige et paramètre}, le code \emph{rend}) est le plus
\textbf{fiable} : aucune erreur de HTML généré, des dimensions toujours exactes, la
charte toujours respectée. La créativité vient du copy et des choix de composition
(image, pastille, thème), guidés par le brief ; le cadre visuel reste verrouillé.

\textbf{Style par défaut :} le menu de couleur de fond propose le \textbf{bleu St
Patricks} \texttt{\#253081} par défaut ; sans image demandée (1.3 vide), on obtient
...
\texttt{offre\_type} et le gabarit rend chacun avec le style adapté, toujours dans le
bloc jaune de la charte. Pour les pourcentages, le signe « - » est ajouté selon le
\textbf{sens} du message (présent pour « Jusqu'à 20 \% », absent pour « Économisez
20 \% »). En option (\texttt{offre\_pastille}), la remise s'affiche en \textbf{badge
de coin} au lieu du bloc central. Seul le \textbf{message} est affiché, jamais le
détail des paliers de la mécanique, qui reste sur la page de destination.
...
Elle rédige le copy et \textbf{déduit} les paramètres structurés (catégorie, type
d'offre, besoin d'image, thème transversal, pastille). \textbf{Ce qu'elle ne fait
pas :} choisir la mise en page (le \textbf{type} de bannière se déduit du format),
choisir la couleur de fond (champ du formulaire), générer du HTML ou une image
matricielle. Sa sortie est \textbf{uniquement un JSON} ; c'est le code Python qui
l'injecte dans un gabarit fixe, rendu en PNG par Playwright. Cela garantit l'absence
d'erreur de parsing, des dimensions toujours exactes et le respect strict de la charte.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Bouton d'action :} détection depuis le brief (1.3) validée par le client.
\end{itemize}
...
\hrule
\vspace{0.5em}
{\small\itshape Fin du cahier de conception (version 0.4). Version 0.4 : dimensions
du site confirmées ; couleur de fond choisie au formulaire (et non par l'IA) ; logo
sur les pubs selon le fond (détourage automatique) ; banque d'images transverses par
thème (ex. Black Friday) avec promo par défaut ; pastille de remise ; signe des
pourcentages géré selon le sens du message.}
